...
def md_heading(title: str, depth: Number, html_id: Union[bool, str] = False) -> str:
    """
        Filter. display heading menu, heading number automatically calculated 
        from previous heading and depth provided
    """
    global md_toc
    global md_auto_generated_heading
    if not(html_id):
        md_auto_generated_heading = md_auto_generated_heading + 1
        html_id=f'autogenerated_heading_{md_auto_generated_heading}'
    ...
    for curDepth in range(depth+1, max(md_headings, key=int)+1 if md_headings else depth+1):
    ...
    md_toc[headingNumbers]={'depth': depth, 'menu': toc_menu}
...
def md_get_toc() -> str:
    ...
    for heading in md_toc:
        ...
        indent=repeat_str('  ', menu['depth'] - secondHeadingDepth)
